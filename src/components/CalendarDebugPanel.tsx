...
export default function CalendarDebugPanel({
  ...
  return (
    <details className="mt-10 rounded-lg border border-slate-200 bg-white text-sm">
      <summary className="cursor-pointer select-none p-3 font-medium text-slate-600">
        ...
        <table className="w-full min-w-[640px] text-left text-xs">
          ...
          <tbody>
            {calendars.map((cal) => {
              ...
              return (
                <tr key={cal.id} className="border-t border-slate-100">
                  <td className="py-1 pr-3 font-medium text-slate-700">
                    {cal.summary}
                    {cal.primary ? " (primary)" : ""}
                  </td>
                  <td className="py-1 pr-3 break-all text-slate-400">{cal.id}</td>
